...
void ExternalCommandPipe::SetWatchdogTimeout(DWORD timeoutMs)
{
    watchdogTimeoutMs = timeoutMs;
}

void ExternalCommandPipe::SetJoinCallback(JoinCallback cb)
{
    onJoin = std::move(cb);
}

void ExternalCommandPipe::SetLogCallback(LogCallback cb)
{
    onLog = std::move(cb);
}

void ExternalCommandPipe::SetDebugCallback(DebugCallback cb)
{
    onDebug = std::move(cb);
}
...
bool ExternalCommandPipe::Start()
{
    ...
    Log("[ECPIPE] Started on pipe: " + pipeName);
    return true;
}

void ExternalCommandPipe::Stop()
{
    if (!running.load())
        return;
    ...
    if (listenerThread.joinable())
    {
        ...
        if (waitResult == WAIT_TIMEOUT)
        {
            ...
            Log("[ECPIPE] Listener thread did not exit in time, detaching.");
...
void ExternalCommandPipe::SendResponse(const std::string& cmd, const nlohmann::json& args)
{
    std::lock_guard<std::mutex> lock(writeMutex);
    if (hCurrentPipe == INVALID_HANDLE_VALUE)
        return;
...
void ExternalCommandPipe::ListenerLoop()
{
    while (running.load())
    {
        // ── 创建管道实例 ──
        ...
        {
            Log("[ECPIPE] CreateNamedPipe failed: " + std::to_string(GetLastError()));
        ...
        while (running.load())
        {
            ...
            {
                Log("[ECPIPE] Line buffer exceeded 64K, resetting.");
                lineBuf.clear();
            }
        }
...
//        解析失败则回写 error 响应。
void ExternalCommandPipe::ParseAndDispatch(const std::string& line)
{
...
void ExternalCommandPipe::Dispatch(const std::string& cmd, const nlohmann::json& args)
{
    if (cmd == "ping")
    {
    ...
    {
        Log("[ECPIPE] Unknown command: " + cmd);
        nlohmann::json resp;
...
void ExternalCommandPipe::Log(const std::string& msg)
{
    if (onLog)
        onLog(msg);
}
