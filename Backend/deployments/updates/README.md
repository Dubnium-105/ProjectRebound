...
For a static `toolbox` descriptor, add a `vnt_runtime` object whose `vnts_version` and `wrapper_version` exactly match the ToolBox build's `res/vnt/runtime/vnt-runtime-manifest.json`. Administrator-managed ToolBox releases instead include that JSON file as an uncompressed release sidecar; the control plane verifies and extracts these values automatically during release validation. The sidecar is attestation input only and is omitted from the public client manifest, which contains only the uncompressed `rebound_toolbox.exe`. Keep `json` in `DOWNLOAD_ALLOWED_EXTENSIONS` so the sidecar can be uploaded without renaming it.
