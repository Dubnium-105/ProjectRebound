...
func TestPrometheusScrapesOnlyInternalControlPlaneMetrics(t *testing.T) {
	...
	if len(config.ScrapeConfigs) != 1 || config.ScrapeConfigs[0].MetricsPath != "/internal/metrics" ||
...
func TestGrafanaDashboardIsValidJSONAndUsesRequiredMetrics(t *testing.T) {
	...
	for _, metric := range []string{"http_requests_total", "http_request_duration_seconds_bucket", "active_sessions", "relay_allocations_active", "refresh_token_reuse_total"} {
