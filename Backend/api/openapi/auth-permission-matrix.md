...
The bind response includes an integrity nonce for verified sessions. A valid ToolBox PEM/ticket proof persists `integrity_trusted` plus the PEM fingerprint on that database session and mirrors the result to `auth_level=trusted` for token compatibility. Refresh inherits both integrity fields. Later proofs omit the ticket. Every failed proof immediately clears session integrity trust; three consecutive failures revoke the session. VNT authorization reads `integrity_trusted`, not the compatibility auth-level mirror.
...
| List own VNT nodes | Allow when Steam verified | Reject | Reject |
| Create VNT enrollment | Allow only when Steam verified, integrity trusted, entitled, and below quota | Reject | Reject |
| Recover/retire an owned VNT node with Player Access | Allow only with integrity-trusted step-up; recovery also consumes a fresh owner enrollment code | Reject | Reject |
...
| Dedicated servers | `game_servers.read` | `game_servers.register`, `game_servers.drain`, `game_servers.disable` |
