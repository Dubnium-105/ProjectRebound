	...
	for {
		...
		case <-heartbeats.C:
			...
			if err := stream.Send(controlEnvelope("TrafficReport", map[string]any{
				...
				"bytes_received_total":     strconv.FormatUint(snapshot.BytesReceived, 10),
				...
				"bind_init_total":          strconv.FormatUint(snapshot.BindInit, 10),
				"bind_challenge_total":     strconv.FormatUint(snapshot.BindChallenge, 10),
				"cookie_invalid_total":     strconv.FormatUint(snapshot.CookieInvalid, 10),
				"token_invalid_total":      strconv.FormatUint(snapshot.TokenInvalid, 10),
				"token_replay_total":       strconv.FormatUint(snapshot.TokenReplay, 10),
				"packet_auth_failed_total": strconv.FormatUint(snapshot.AuthenticationFailed, 10),
				"packet_replay_total":      strconv.FormatUint(snapshot.ReplayDropped, 10),
				"packet_too_large_total":   strconv.FormatUint(snapshot.PacketTooLarge, 10),
				...
				"load_ratio":               strconv.FormatFloat(snapshot.LoadRatio, 'g', -1, 64),
				"goroutines":               strconv.Itoa(snapshot.Goroutines),
				"memory_bytes":             strconv.FormatUint(snapshot.MemoryBytes, 10),
			})); err != nil {
				return err
			}
