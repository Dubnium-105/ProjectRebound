...
| 文件 | 变更 |
|------|------|
| `ProjectReboundMainDLL/Loadout/LoadoutSerializer.h` | 新增 `NormalizeLoadoutFormat()`、`DecodeWeaponArchiveRaw()`、`ApplySkinAndOrnament()` 声明 |
| `ProjectReboundMainDLL/Loadout/LoadoutSerializer.cpp` | 新增 ~250 行：格式归一化、最小 protobuf 解码器、皮肤/装饰品映射 |
| `ProjectReboundMainDLL/Loadout/LoadoutManager.cpp` | `OnRoleSelectionConfirmed()` 调用 `NormalizeLoadoutFormat()` 统一处理新旧格式 |
| `ProjectReboundMainDLL/Loadout/LoadoutApplication.cpp` | 修复 `GetFieldModManager()` 重复 return 语句 |
...
| P1 | Metaserver StartUnityMatchmaking 改造 | Metaserver 已并入本 repo；需在 `Metaserver/` 中对接 `/matchmaking/enqueue` |
