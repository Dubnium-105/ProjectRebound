import { tr } from "../i18n";
import type { BaseRecord, CrudFilter, DataProvider, HttpError } from "@refinedev/core";
...
function toHttpError(error: unknown): HttpError {
    ...
    return {
        message: error instanceof Error ? error.message : tr("\u8BF7\u6C42\u5931\u8D25\u3002"),
        statusCode: 500
    };
}
export const dataProvider = {
    ...
    async getOne({ resource, id }) {
        try {
            const item = await apiRequest<Record<string, unknown>>(`${pathFor(resource)}/${encodeURIComponent(String(id))}`);
    ...
    async create({ resource, variables }) {
        try {
            ...
            const item = resource === "invite-codes" &&
    ...
    async update({ resource, id, variables }) {
        try {
            const result = await apiRequest<Record<string, unknown>>(`${pathFor(resource)}/${encodeURIComponent(String(id))}`, {
                method: "PATCH",
                body: JSON.stringify(variables)
            });
            const item = resource === "players" &&
    ...
    async deleteOne() {
        throw toHttpError(new ApiError(405, "METHOD_NOT_ALLOWED", tr("\u8BE5\u8D44\u6E90\u4E0D\u652F\u6301\u76F4\u63A5\u5220\u9664\u3002")));
    },
    getApiUrl() {
        return "/v1/admin";
    }
