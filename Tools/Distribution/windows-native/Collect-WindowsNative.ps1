...
if ([string]::IsNullOrWhiteSpace($OutputPath)) {
    $OutputPath = Get-NativeDefaultEvidencePath -PackagePath $PackageRoot -LeafName 'collect.json'
}
...
try {
    ...
    $evidence.tool = 'ProjectRebound.WindowsNativeDistribution.Collect'
    $evidence.collection = [ordered]@{
        scope = 'package_bytes_host_dependency_versions_and_hashes_only'
        collected = @('package-manifest.json', 'package file SHA-256/size', 'fixed game SHA-256/size', 'Steam/WebView2/MSVC presence/version summaries', 'exact game process count')
        omitted = @('account identifiers', 'Steam identifiers', 'access/refresh tokens', 'join grants', 'tickets', 'raw client logs', 'Steam/config files', 'command lines', 'environment variable values')
        status = 'PASS'
    }
    ...
    $errorEvidence = [ordered]@{
        schema_version = 1
        tool = 'ProjectRebound.WindowsNativeDistribution.Collect'
        tool_version = '1.0'
        ...
        collection = [ordered]@{ status = 'ERROR'; raw_logs_collected = $false; credentials_collected = $false }
