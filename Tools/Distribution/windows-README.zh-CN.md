...
r9 实测已到达 `authority_ready`，但主客户端命名管道事务返回 Windows OS 错误 231，导致原生启动失败。r10 缩短该事务并及时释放管道，同时保留严格准入；不会绕过原生检查。r10 的多人原生结果必须重新采集；桌面启动或单独到达 `authority_ready` 都不能作为通过。

房主冻结前的运行时检查、启动时再次检查、房主准备和切换队伍按钮继续保留。本轮所有人统一使用 r10 包。包绑定现有已部署后端版本，Payload 字节不变；`release_ready=false`。
