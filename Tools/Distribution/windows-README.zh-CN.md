...
r13 实测成员已到 `transport_ready`，但主从机仍停在主菜单；房主随后报 `world_ready_timeout`，成员启动因房主终止而取消。本机成员原生日志有管道监听和登录完成，没有管道客户端连接或受管入局命令。r14b 将严格启动的首次 PID 绑定管道握手移到通用窗口等待门之前，继续等待原生登录完成及签名准入；同时传播原生 `InitListen` 的真实失败，并保留房主 World 等待的具体结构条件。现有证据尚不能确定本次房主超时究竟缺少哪项条件。原生 `RoundState=InvalidState` 仍是首次开局前的空闲数据；主菜单或单独的 `authority_ready` 不算进入战局。

房主冻结前的运行时检查、启动时再次检查、房主准备和切换队伍按钮继续保留。本轮所有人统一使用 r14b 包，完全退出旧版后安装本包新的 Payload，再创建全新大厅。包绑定现有已部署后端版本；`release_ready=false`。
...
原生实机结果以本包对应的实际执行回执为准。r13 的多人测试失败是历史证据；旧版本结果或单次窗口启动都不能作为本包通过的依据。三机原生准入、角色出生与操作、准入负例、清理和下一局复用必须分别记录；缺少对应执行证据时保留 `NOT_RUN` 或具体 `BLOCKED`。
