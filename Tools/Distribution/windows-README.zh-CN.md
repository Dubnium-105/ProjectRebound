...
r10 实测在启动期间收到 `SESSION_REVOKED`，随后取消游戏启动；后台记录与正常登录凭据轮换相关。r11 修复轮换后旧请求无法重试的问题，并让账户校验和启动续租取得当前会话凭据。真正撤销、登出、换号和严格原生准入检查继续生效。r11 的多人原生结果必须重新采集；桌面启动或单独到达 `authority_ready` 都不能作为通过。

房主冻结前的运行时检查、启动时再次检查、房主准备和切换队伍按钮继续保留。本轮所有人统一使用 r11 包，完全退出旧版后创建全新大厅。包绑定现有已部署后端版本，Payload 字节不变；`release_ready=false`。
...
原生实机结果以本包对应的实际执行回执为准。r9 的 `authority_ready`/OS 231 失败只是历史证据，不能代表 r10 结果；旧版本失败或单次窗口启动结果也不能套用到本包。三机原生准入、角色出生与操作、准入负例、清理和下一局复用必须分别记录；缺少对应执行证据时保留 `NOT_RUN` 或具体 `BLOCKED`。
