...
The runtime checks before roster freezing and native startup, Owner Ready, and team switching remain available. Everyone must use the same r14 package, fully exit old versions, install this package's new Payload and create a new lobby. The package binds the existing deployed backend version; `release_ready=false`.
