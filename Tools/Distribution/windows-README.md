...
The reported r12 run reached HOST listening, then failed with `missing field attempt_id`; MEMBER candidate negotiation failed with `Invalid connection candidate` and the game remained at the main menu. R13 accepts the Payload's empty diagnostic scope objects as absent, preserves malformed nonempty scope rejection, and classifies connection addresses according to the existing backend contract. It prefers one valid STUN SRFLX candidate instead of publishing competing endpoints of the same type. The original rejected frame and candidate fields were not captured, so their exact contents remain unknown. Native `RoundState=InvalidState` remains valid first-round idle data. Desktop startup, the main menu, or `authority_ready` alone cannot establish entry into a match.

The runtime checks before roster freezing and native startup, Owner Ready, and team switching remain available. Everyone must use the same r13 package for this round, fully exit old versions and create a new lobby. The package binds the existing deployed backend version and unchanged Payload bytes; `release_ready=false`.
...
Native hardware results must come from execution receipts for this exact package. The r12 multiplayer failure is historical evidence; earlier build results or a window-opening check cannot establish acceptance for this build. Three-machine admission, character spawn and control, rejection cases, cleanup, and reuse in a new match need separate evidence. Keep missing executions as `NOT_RUN`, or record a concrete `BLOCKED` condition.
