...
当前所有公开入口和跨组件规范文档都登记在 [`bilingual-docs.txt`](bilingual-docs.txt)，并由 CI 检查。新增公开文档必须在同一提交中加入登记并同时创建两个文件。

`docs/archive/` 下的历史快照保持冻结，不做翻译。`docs/testing/` 中的版本证据可以保留采集证据时使用的语言，但索引和 Release Gate 结论必须双语。靠近实现的组件说明在发生实质修改前可以暂时保持单语；实质修改时应遵循本标准，或链接到已登记的双语规范文档。
