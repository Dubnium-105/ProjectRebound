...
For `/connectServer`, MetaTunnel injects the bearer token. MetaServer ignores
legacy `playerId` and `loginToken` fields and derives player ID, auth session,
account state, client version, and protocol version server-side. It stores a
