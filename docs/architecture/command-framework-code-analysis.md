# CommandFramework C++ function/structure/code analysis guide
...
| --- | --- | --- |
| `Payload/Communication/CommandProtocol.h/.cpp` | Constants, request values, parsing, encoding, target validation | Portable C++ |
| `Payload/Communication/CommandFramework.h/.cpp` | ACL, pipe, Overlapped I/O, lifecycle, dispatch | Windows |
| `Payload/ClientLogic/ClientLogic.h/.cpp` | Thread-safe join queue and game-thread state machine | Windows and Unreal SDK |
| `Payload/Hooks/Hooks.cpp` | Login thread identity and post-`ProcessEvent` command pump | Unreal hook |
| `Payload/dllmain.cpp` | Dependency injection, startup, explicit shutdown export | Windows DLL |
| `Payload/Tests/CommandProtocolTests.cpp` | Parser, validator, and encoder boundaries | Portable |
| `Payload/Tests/CommandFrameworkTests.cpp` | Real Windows pipe integration | Windows |
| `Payload/Tests/CMakeLists.txt` | Isolated test build | CMake |

Both Communication sources are explicit items in `Payload/Payload.vcxproj`. The protocol
...
`ClientLogic.cpp` protects `pendingTarget`, `currentTarget`, stage, and deadline with
...
Client startup configures a temporary `unique_ptr<CommandFramework>` and publishes a raw
...
```powershell
cmake -S Payload/Tests -B build/command-pipe-tests
cmake --build build/command-pipe-tests --config Release
ctest --test-dir build/command-pipe-tests -C Release --output-on-failure

msbuild Payload/Payload.vcxproj /m /p:Configuration=Release /p:Platform=x64
...
```powershell
rg -n "ParseFrame|ValidateMatchTarget|EncodeFrame" Payload/Communication
rg -n "Start\(|Stop\(|ListenerLoop|ReadClient|SendResponse" Payload/Communication
