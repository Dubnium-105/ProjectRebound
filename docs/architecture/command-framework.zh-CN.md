...
- `Payload/Communication/CommandProtocol.*`：可移植的帧解析、编码和比赛目标校验；
- `Payload/Communication/CommandFramework.*`：Windows 命名管道安全、Overlapped
  I/O、生命周期、分发和响应；
- `Payload/ClientLogic/ClientLogic.*` 与 `Payload/Hooks/Hooks.cpp`：线程安全的加入队列
  和游戏线程执行。
