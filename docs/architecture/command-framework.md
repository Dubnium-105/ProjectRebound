...
- `Payload/Communication/CommandProtocol.*`: portable framing, parsing, encoding, and
  match-target validation;
- `Payload/Communication/CommandFramework.*`: Windows named-pipe security, Overlapped
  I/O, lifecycle, dispatch, and responses;
- `Payload/ClientLogic/ClientLogic.*` plus `Payload/Hooks/Hooks.cpp`: thread-safe join
