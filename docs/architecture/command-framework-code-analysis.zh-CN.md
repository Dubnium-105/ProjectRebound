# CommandFramework C++ 函数—结构—代码分析指南
...
| 文件 | 主要职责 | 平台依赖 |
| --- | --- | --- |
| `Payload/Communication/CommandProtocol.h/.cpp` | 常量、请求结构、帧解析/编码、目标校验 | 纯 C++ |
| `Payload/Communication/CommandFramework.h/.cpp` | ACL、命名管道、Overlapped I/O、生命周期、分发 | Windows |
| `Payload/ClientLogic/ClientLogic.h/.cpp` | 线程安全加入队列、游戏线程状态机 | Windows、Unreal SDK |
| `Payload/Hooks/Hooks.cpp` | 登录线程标识、`ProcessEvent` 后置命令泵 | Unreal Hook |
| `Payload/dllmain.cpp` | 依赖注入、启动、显式卸载入口 | Windows DLL |
| `Payload/Tests/CommandProtocolTests.cpp` | 纯协议边界与编码测试 | 可移植 |
| `Payload/Tests/CommandFrameworkTests.cpp` | 真实 Windows 管道集成测试 | Windows |
| `Payload/Tests/CMakeLists.txt` | 独立测试构建入口 | CMake |

`Payload/Payload.vcxproj` 和 `.filters` 显式包含两个 Communication 源文件；协议层因此既可随
...
`ClientLogic.cpp` 的内部状态由 `connectMutex` 保护：
...
客户端初始化创建临时 `unique_ptr<CommandFramework>`，注入 Join、Log 和 Debug 回调；只有
`Start()` 成功后才发布为进程生命周期的全局指针，启动失败会自动析构，不泄漏半初始化
对象。
...
| CommandFramework 监听线程 | accept、read、parse、dispatch、write |
...
`CommandFrameworkTests.cpp` 使用真实 `CreateFileW` 客户端覆盖：
