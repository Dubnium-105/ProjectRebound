...
这里保存已经被当前实现取代、但仍有设计追溯价值的快照。归档内容可能包含不存在的目录、旧端口、旧服务地址、SQLite/C# 后端和已完成的实施计划，请勿用于当前构建、API 集成或生产部署。
...
| `loadoutmanager-metaserver-rework.md` | 指向已不存在的 BoundaryMetaServer | 当前 Payload/控制面实现 |
| `metaserver-client-matchserver-request-m-reflective-puppy.md` | 旧 MatchServer 分阶段方案 | 当前 Go 控制面与 Edge Relay |
