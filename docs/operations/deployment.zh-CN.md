...
主机准备、防火墙、FRP/HAProxy、证书、DNS 和首次注册的权威流程见 [Debian 部署与运维](deployment-guide.md)。CI/CD 发布不可变 `sha-<commit>`、语义版本镜像、provenance 和构建记录；生产主机拉取这些产物，不在目标机编译。见 [CI/CD](ci-cd.md)。
...
发布环境和回滚行为见 [V1.1 发布与回滚](release-and-rollback.md)。完成 [Release Checklist](../testing/v1.1/release-checklist.md)，当[测试报告](../testing/v1.1/test-report.md)或[恢复报告](../testing/v1.1/restore-test-report.md)仍有必须执行的 `NOT_RUN`/`FAIL` 门禁时，不得提升版本。
