...
- Download management uses same-host MinIO by default. Keep the server-only `DOWNLOAD_S3_ENDPOINT=http://minio:9000` and `DOWNLOAD_PUBLIC_PROBE_BASE_URL=http://minio:9000/<bucket>`; replace the example hosts in `MINIO_S3_SITE`, `DOWNLOADS_SITE`, `DOWNLOAD_S3_UPLOAD_ENDPOINT`, `DOWNLOAD_PUBLIC_BASE_URL`, and `MINIO_CORS_ALLOWED_ORIGINS` together, then point the S3 and download DNS A/AAAA records at the Control Plane. The public bases must include the bucket name, and the MinIO Console must remain reachable only through an SSH tunnel to `127.0.0.1:MINIO_CONSOLE_PORT`.
...
- The production control-plane image includes the canonical LF ToolBox certificate at `/usr/share/projectrebound/toolbox-signer.pem`. Every deployment compares the image SHA-256 with the versioned deployment asset and fails closed on drift; do not replace it with a host mount or reformat/base64-transform it. Standalone deployments using `TOOLBOX_PUBKEY_PATH` must supply the same exact bytes. `INTEGRITY_CHALLENGE_TTL_SECONDS` defaults to 120 and `INTEGRITY_MAXIMUM_FAILURES` must remain 3 unless the client and incident-response policy are updated together.
