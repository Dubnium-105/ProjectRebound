...
- Download management uses same-host MinIO by default. Keep the server-only `DOWNLOAD_S3_ENDPOINT=http://minio:9000`; replace the example hosts in `MINIO_S3_SITE`, `DOWNLOADS_SITE`, `DOWNLOAD_S3_UPLOAD_ENDPOINT`, `DOWNLOAD_PUBLIC_BASE_URL`, and `MINIO_CORS_ALLOWED_ORIGINS` together, then point the S3 and download DNS A/AAAA records at the Control Plane. The public base must include the bucket name, and the MinIO Console must remain reachable only through an SSH tunnel to `127.0.0.1:MINIO_CONSOLE_PORT`.
