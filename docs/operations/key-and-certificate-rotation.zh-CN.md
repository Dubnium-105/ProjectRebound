...
[English](key-and-certificate-rotation.md) | 简体中文
...
监控必须按动态节点清单展示 `control_connected`、心跳年龄、证书到期时间和剩余有效期，并在进入续签窗口后证书仍未更新时告警。证书过期、身份撤销和普通网络断线是不同故障：只有普通断线应先等待内建重连；过期或撤销必须使用受控的新身份流程。连续运行和恢复边界见 [Relay 连续在线与恢复](relay-continuity.zh-CN.md)。
