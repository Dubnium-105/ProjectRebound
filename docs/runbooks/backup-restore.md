...
Use [PostgreSQL and key backup/restore](../backup-restore.md) for commands and retention policy.
...
5. Record backup ID/hash, RPO, start/end/RTO, schema/image versions, row counts, failures, and operator in `docs/v1.1/restore-test-report.md`. Publish the textfile success timestamp only after every required check passes.
