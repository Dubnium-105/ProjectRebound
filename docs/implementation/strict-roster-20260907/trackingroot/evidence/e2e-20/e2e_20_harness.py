...
class Harness:
    ...
    def updater_fixture(self) -> dict[str, Any]:
        fixture = self.root / ".tmp" / "strict-roster-20260907" / "source-a049d56" / "Tools" / "Release" / "test_updater_transactions.py"
        return self.run(
    ...
    def updater_interruption(self) -> dict[str, Any]:
        ...
        needle = "    Stop-Process -Id $ParentProcessId -Force -ErrorAction SilentlyContinue"
        ...
        paused = script.replace(needle, f"    Set-Content -LiteralPath '{marker_ps}' -Value 'staged' -Encoding UTF8\n    Start-Sleep -Seconds 30\n{needle}", 1)
        ...
        try:
            updater = subprocess.Popen(
                [powershell, "-NoLogo", "-NoProfile", "-NonInteractive", "-ExecutionPolicy", "Bypass", "-File", str(interrupted_script), "-ParentProcessId", str(parent.pid), "-Source", str(staged), "-Target", str(target), "-LogPath", str(log_path), "-ExpectedSize", str(staged.stat().st_size), "-ExpectedSHA256", digest],
        ...
        recovery = subprocess.run(
            [powershell, "-NoLogo", "-NoProfile", "-NonInteractive", "-ExecutionPolicy", "Bypass", "-File", str(recovery_script), "-ParentProcessId", str(recovery_parent.pid), "-Source", str(staged), "-Target", str(target), "-LogPath", str(log_path), "-ExpectedSize", str(staged.stat().st_size), "-ExpectedSHA256", digest],
    ...
    def provenance(self) -> None:
        script = self.root / ".tmp" / "strict-roster-20260907" / "source-a049d56" / "Tools" / "Release" / "strict_roster_provenance.py"
        ...
        if collect_step["status"] == "PASS" and gate["status"] == "PASS" and collect_step["log_path"]:
            try:
                payload = json.loads((self.tmp / "provenance-collect.log").read_text(encoding="utf-8").split("--- stdout ---\n", 1)[1].split("\n--- stderr ---", 1)[0].strip().splitlines()[-1])
    ...
    def finish(self, snapshots: dict[str, Any]) -> dict[str, Any]:
        ...
        result = {
            ...
            "limitations": [
                ...
                "The updater interruption case hard-killed the copied PowerShell script. Current product recovery restores the prior target but leaves the interrupted pending candidate; this is recorded as PARTIAL and was not repaired in a frozen-source E2E run.",
