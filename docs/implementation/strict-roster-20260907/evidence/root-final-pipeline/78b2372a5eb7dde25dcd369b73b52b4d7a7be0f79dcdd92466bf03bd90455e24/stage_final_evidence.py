...
for index in range(0, len(paths), 60):
    ...
    # Existing index entries may still hold normalized bytes even after -text
    # was introduced. Force rereading the current bytes instead of trusting
    # Git's unchanged-file stat cache. The first add includes new paths too.
    subprocess.run(['git','add','--renormalize','-f','--',*paths[index:index+60]],cwd=repo,check=True)
