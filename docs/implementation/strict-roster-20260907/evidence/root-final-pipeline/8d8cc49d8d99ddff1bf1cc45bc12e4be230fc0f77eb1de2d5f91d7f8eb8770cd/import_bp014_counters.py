    ...
    for issue in report.get('issues',[]):
        if issue['id']=='BP-014':
            ...
            for value in issue.get('evidence',[]):
                old=Path(value)
                if old.parent==root/'evidence/final-supplements'/base.name and old.name.startswith('go-test-toolbox-private-counters'):
                    replacement=retained(base/old.name)
                    if sha(old)!=sha(replacement): raise ValueError('Helper log alias changed evidence bytes')
                    value=str(replacement)
                previous.append(value)
            issue['evidence']=list(dict.fromkeys(previous+[str(p) for p in refs]))
